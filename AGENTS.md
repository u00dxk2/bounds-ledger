...
- `npm test` — network-free self-tests (6, all wired into CI; `reverify.test.mjs` fails if you add another without a workflow step)
...
- **Never commit secrets** — this repo is heading for a public flip. Credentials come from user-scope env at run time, never from repo files or CI secrets.
- **Don't hand-edit** `ledger/teorth-optimizationproblems/**` (machine mirror; refresh only via `--snapshot`) or the `generated: true` pins in `ledger/claims.json` (rewritten wholesale by `extract-pins.mjs`). Only hand claims C-1…C-9 are hand-edited.
