...
**Two mistakes of mine, recorded because they cost something.** I caused the rate limit: two dispatches inside ten minutes, roughly 220 fetches each, on top of the daily scheduled run. Dispatching this workflow spends a shared budget the *scheduled* run depends on, and the first run had already answered the question. Separately, a throwaway debug one-liner's escaping failed to strip the step's `gh issue create` before executing it, and it filed a real issue (#16) against the public repo; closed within minutes. Its title was `Check error: bounds-ledger could not reach 2 cited source(s)` — an accidental end-to-end proof that the fix works through the real `gh` path, which does not make it less of a mistake.
