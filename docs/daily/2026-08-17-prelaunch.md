...
- **CI:** scheduled run 2026-08-17 09:27Z on `main` @ `e2b9935` — **success**. **That run predates the GitHub outage by over four hours** (incident opened 13:40:03Z), so it sits outside the window in which CI conclusions must be read as UNKNOWN — the streak day rests on a trustworthy read. All 25 scheduled runs in the streak fired at roughly 09:2x UTC on their respective days, i.e. every one of them pre-outage. Verified with `check-ci-status.mjs --workflow reverify.yml`, exit 0, on a completed non-scheduled run matching HEAD. Two `workflow_dispatch` failures today on feature branches, both HTTP 429, neither on `main`.
