...
**THE NUMBER THAT WILL LIE TO YOU** — **the six failed CI runs dated today, and the eleven issues closed.** A cold reader will conclude either that a record moved or that G-1's green streak — the goal of becoming a re-verified steward of a drifting record inventory — broke. Neither happened. Both failures were `workflow_dispatch` runs on *feature branches*, triggered by me to validate a held merge; today's **scheduled** run on `main` (`e2b9935`, 09:27Z) passed, so the streak stands at day 25 of 30. And the closed `Drift:` issues were transport failures (429/502/503): the sources were never read, so nothing could be found to have moved. Read the `event` and `headBranch` fields before reading a conclusion; the distinction is invisible in a bare red/green count. **Final tally, and none of it is a record moving:** `schedule` 1 success (09:27Z — the streak day, pre-outage); `workflow_dispatch` 2 failures (14:32Z, 14:41Z); `push` 6 runs, 4 failures then **2 successes** once transport recovered (`90259e1` 16:28Z, `ee1fa2c` 16:33Z); `pull_request` 2 successes. Every failure was transport (429/502/503) with **0 `BROKEN` lines and 0 mirror verdicts** in each log. Of the eleven issues closed: five were false `Drift:` filings from those transport failures, five were *resolved* drifts from 11/12/14 August that nobody had closed at resolution time, and one (#16) I filed by accident. The cause of the transport failures was **GitHub's own critical outage from 13:40:03Z**, not our request volume — see the third mistake below, where I got that wrong first.
...
- **CI:** scheduled run 2026-08-17 09:27Z on `main` @ `e2b9935` — **success**. **That run predates the GitHub outage by over four hours** (incident opened 13:40:03Z), so it sits outside the window in which CI conclusions must be read as UNKNOWN — the streak day rests on a trustworthy read. All 25 scheduled runs in the streak fired at roughly 09:2x UTC on their respective days, i.e. every one of them pre-outage. Verified with `check-ci-status.mjs --workflow reverify.yml`, exit 0, on a completed non-scheduled run matching HEAD. **main is GREEN at HEAD `ee1fa2c`** — run 32046168575, a completed non-scheduled success, read from a `main` checkout with `HEAD == origin/main` confirmed first. That run performed the full network check (113 files, 233 claims) on a runner, so it is positive, self-validating evidence. GitHub's incident was still officially open at 16:36Z ("strong signs of recovery"), and the fleet rule is to record in-window conclusions as UNKNOWN — but note the asymmetry the rule exists for: an outage manufactures false **reds**, never false greens, because a run cannot pass by fetching 113 files unless it actually fetched them. Earlier today: 6 failures across `workflow_dispatch` and `push`, all transport (429/502/503), none of them a record moving.
