...
- **Gates**: `npm run verify` run at `76554fdd` AFTER today's ledger edits, because `check:deferrals` reads `continuity/items.json` and a receipt taken before the edit cannot say which leg is red now. Its own stamp warned the tree was dirty at stamp time, which is true and is why this line names the edit rather than claiming a clean stamp — `tmp/.verify-receipt.json`.
