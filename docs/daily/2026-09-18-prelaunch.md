...
- **HEAD**: `5603abd` "Hygiene 2026-09-18: the day's report, tomorrow's primer, the phase-4 record, and the Gotchas move" — `git -C . log -1 --format=%h%x20%s`, read 19:42Z. That commit is the one that landed this file; this line was written by the follow-up commit that carries the Gates line below.
...
- **Gates**: `npm run verify` **exit 0 at `5603abd`**, stamped 19:41:24Z, `failedGates []`, with `check:brief` UNVERIFIABLE — the permanent state recorded on `A-41` (the decision that the brief check is script-unverifiable since `/t/*` went Google-session-only) — `tmp/.verify-receipt.json`. It ran THREE times today, each over a different commit and none of them a re-run of the same tree: `5ebdb54` at 15:26Z (the morning cadence), `70ebd63` at 16:14Z (the P2 ledger batch), `5603abd` at 19:41Z (this close batch). This line names the commit that landed the report, which is the one exception the appendix's own rule allows, and it is why this follow-up commit exists.
