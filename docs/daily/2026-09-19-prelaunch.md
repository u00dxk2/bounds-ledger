...
- **CI**: **UNKNOWN at `53c3abd`**, not green and not pending — `node scripts/sky.mjs check-ci-status.mjs --workflow reverify.yml --sha <full 40-char sha>` printed `UNKNOWN — 1 run(s) for HEAD still in progress` and exit 2 at 16:23Z, and that checker says in terms that UNKNOWN is not a pass. GREEN was read at `645457a` (1 completed non-scheduled success, 0 failures, 0 pending) at 16:02Z. **Clearing condition for this line: re-run that command; when it returns GREEN or RED at `53c3abd`, this UNKNOWN is spent and should be replaced, not carried.**
- **Gates**: `npm run verify` exit 0, receipt `{exitCode: 0, sha: 645457a…, at: 2026-09-19T16:15:54.513Z}` with `failedGates: []` — the receipt sits at the commit it was taken at, and `53c3abd` landed after it with `npm test` and `render-ramsey --check` run green on the working tree before the commit.
