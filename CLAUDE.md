...
- `ledger/claims.json` — claim-level value pins (source URL + exact expected string per claim). 8 hand claims (C-1…C-8; C-8 pins problem 36's entry in `teorth/erdosproblems` `data/problems.yaml` — the A-6 second surface) + 216 generated pins (`pin:<file>:U|L`, `generated: true`) covering all 109 constants. Hand-edit only the hand claims; generated pins are rewritten wholesale by `extract-pins.mjs`.
