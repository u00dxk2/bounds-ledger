...
- **A `manual: true` claim reports UNVERIFIED, never green.** The rule stands, but as of 2026-07-25 **no claim uses it** — erdosproblems.com turned out to be fetchable (see Gotchas), so C-7 was automated and the last UNVERIFIED disappeared. `manual: true` is now the *fallback for a source we genuinely cannot check*, not a standing state. Before marking anything manual, prove the fetch fails; a stale "we can't check this" is worse than no claim, because it parks a question forever.
...
- **~~erdosproblems.com 403s automated fetch.~~ FALSE as of 2026-07-25 — the convention was stale.** A Node default-UA `fetch` of `https://www.erdosproblems.com/36` returns **HTTP 200**, 35185 bytes, with the bound and the last-edited date both readable in the body (Playwright confirms the same content). Whatever blocked us earlier no longer does. Consequence: **C-7 is no longer `manual: true`** and a companion **C-8** pins the last-edited line — both re-run in CI daily, so two of W-3's three acknowledgement triggers are now mechanical. The lane has **zero UNVERIFIED claims**. If the block ever returns it surfaces as UNREACHABLE (a legible check error since A-5), not as a silent green. **Don't re-add `manual: true` without re-testing the fetch first.**
