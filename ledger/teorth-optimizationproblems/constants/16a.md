...
- The simpler "one-log" Brezis–Gallouet inequality $\lVert u\rVert\_{\infty}^2 \le C\lVert \nabla u\rVert\_{2}^2(\ln\delta + K)$ has infimum $C = \frac{1}{4\pi}$, but this infimum is **not attained** with any finite $K$—the log-log correction is necessary.
