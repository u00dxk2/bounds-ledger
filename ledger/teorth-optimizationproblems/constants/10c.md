...
## Certificate for the $7/\sqrt{17}$ lower bound

Use the following $17$ by $17$ sign matrix. It has $16$ distinct rows in $17$ columns; the last row repeats the first (a duplicate row does not change the discrepancy) purely to make the matrix square.
...
As in the previous certificates, for a sign vector $x$ let $S$ be the set of columns where $x$ is $-1$, and for row $i$ let $T_i$ be the set of columns where that row is $-1$ (columns numbered $1$–$17$):
...
Then $(Ax)_i = 17 - 2\lvert S\triangle T_i\rvert$, so $\lvert (Ax)_i\rvert \ge 7$ exactly when $S$ is within Hamming distance $5$ of $T_i$ or of its complement. The $16$ sets $T_i$, together with their complements, thus form radius-$5$ Hamming balls that cover all of $\{0,1\}^{17}$; equivalently,

$$
\min_{x\in\{\pm 1\}^{17}}\lVert Ax\rVert_\infty = 7 .
$$

This was checked by exhaustive evaluation over all $2^{17}=131072$ sign vectors (and, independently, by verifying the radius-$5$ covering of the $17$-cube). Hence $\mathrm{disc}(A)=7$ and

$$
C_{10c}\ge \frac{7}{\sqrt{17}}\approx 1.697749 .
$$
