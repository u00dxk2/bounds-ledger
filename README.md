...
**A second area is watched, on its own page: [small Ramsey numbers R(k, l)](https://u00dxk2.github.io/bounds-ledger/ramsey.html).** It holds what Radziszowski's dynamic survey *Small Ramsey Numbers* prints for the two-colour classical Ramsey numbers of its Section 2.1, with the reference the survey credits for each value, and says for each entry whether anyone has read it against the paper it credits. **Its coverage is partial, and the number to read it by is 76, not 72:** of the 76 pairs R(k, l) with 3 ≤ k ≤ 10 and k ≤ l ≤ 15, the committed table carries 72 — R(9, 14), R(9, 15), R(10, 13) and R(10, 14) are absent from it — plus 50 entries from the survey's Table Ib. Five of its values have so far been read against the papers they credit: 2 sound, 2 unresolved, 1 unreachable (`node scripts/ds1-depth.mjs`, as of 2026-09-21). The page's own summary still says "every"; it is frozen until a rebuild of that table from the underlying papers lands, and the correction goes in with it. The values are derived from the journal's PDF by [`scripts/extract-ds1.mjs`](scripts/extract-ds1.mjs), which refuses a layout it cannot place rather than emitting a partial table, and stored in [`ledger/ejc-ds1/section-2-1.json`](ledger/ejc-ds1/section-2-1.json). The survey itself is not copied into this repository: it grants no licence to republish it, so what is stored here is values, reference keys and the PDF's fingerprint.
...
Indicators, which print a figure and no verdict and never gate anything: `npm run depth-audit` (what has been read against its own sources, above), `npm run catches` (record movements per week — a ceiling, and it cannot see the Ramsey area at all), `npm run reports` (outside reports arriving through the per-row links on the page).
