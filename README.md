...
**A second area is watched, on its own page: [small Ramsey numbers R(k, l)](https://u00dxk2.github.io/bounds-ledger/ramsey.html).** It holds what Radziszowski's dynamic survey *Small Ramsey Numbers* prints for the two-colour classical Ramsey numbers of its Section 2.1, with the reference the survey credits for each value, and says for each entry whether anyone has read it against the paper it credits. **What it covers is narrower than the range it names, and the survey is why:** that range — the 76 pairs R(k, l) with 3 ≤ k ≤ 10 and k ≤ l ≤ 15 — is wider than what Table Ia prints, which is 72 of them. The survey gives no value at R(9, 14), R(9, 15), R(10, 13) or R(10, 14). Table Ib adds 50 further upper bounds, every one of them for a pair Table Ia already covers, so it closes none of those four gaps. Five of the 122 credited bounds in Table Ia have been taken up so far: 2 sound, 2 unresolved, 1 unreachable — the last meaning the credited paper could not be opened, not that it was read and disagreed (`node scripts/ds1-depth.mjs`, as of 2026-09-21). The page's own summary said "every" until 2026-09-22, when that one sentence was corrected on the owner's approval; the rest of the page stays as written until a rebuild of that table from the underlying papers lands. The values are derived from the journal's PDF by [`scripts/extract-ds1.mjs`](scripts/extract-ds1.mjs), which places each number by the ruled grid the survey itself draws and refuses a layout it cannot place rather than emitting a partial table — so a cell absent here is a cell the survey leaves blank, not one this reader dropped. The table is stored in [`ledger/ejc-ds1/section-2-1.json`](ledger/ejc-ds1/section-2-1.json). The survey itself is not copied into this repository: it grants no licence to republish it, so what is stored here is values, reference keys and the PDF's fingerprint.

## What it has actually caught

Since the alarm was armed on 2026-07-24, 17 upstream drifts, the most recent on 2026-09-26 — each verified before the mirror was updated, against primary sources where a number moved and against upstream's own content where the change was editorial. **Every one of them is on the mirrored surface** — which is the constants files plus upstream's own `README.md`, where several of these drifts landed. This count reads the mirror's own history, so a new revision of the survey behind the second watched area, small Ramsey numbers, trips that area's own check and never appears in this number; nor would a change to the erdosproblems entries this ledger pins. One row per resolution *story*, not per changed file and not per cycle — the sixteen rows below cover seventeen cycles, because the 2026-08-12 row is two of them. To count them yourself: `git log --oneline -- ledger/teorth-optimizationproblems/manifest.json`, minus the initial-snapshot commit `b5e3ac9` and the mirror-extension commit `3a698b1`, neither of which resolved a drift:
...
| [`3eb95d7`](../../commit/3eb95d7) | The **bounded prime gap constant** arrived, and with it the whole chain `70000000 → 4680 → 600 → 246 → 240` — the last step Stadlmann's, published two days earlier. Its exponent came in the same push. Upstream's own note carries the trap: the exponents of distribution behind these results hold only for smooth moduli, so they do **not** lower-bound `C_66`, "an easy and tempting mistake to make from the abstracts alone". |
| [`c28ffd7`](../../commit/c28ffd7) | Upstream added the unnormalized single-set sum-difference exponent `C_3e` **two minutes before our check ran**. An addition, not a movement. Its published lower pin reads `≈ 1.21` — AlphaEvolve's *inferior* row, not the bound — which is the clearest live example of why a generated pin is a listing position and never a record. |
...
| [`c9b3599`](../../commit/c9b3599) | Editorial, and caught by the CLOSE battery rather than by the scheduled alarm: `npm run verify` exited **1** where this lane's standing red is **3**, and that difference in exit code is what said the failure was upstream and not the brief leg. Upstream swapped one cross-link on `88a.md`, from the Wikipedia article on bounded gaps between primes to the one on Polignac's conjecture. Verified at the live sha before the mirror moved, with the prime-gap prose and the string `246` as the positive control that the fetch was the right document; every bound row byte-identical — `70000000`, `4680`, `600`, `246`, `240`, `2` — with the same citation keys. **The proof that no record moved is a file's ABSENCE from a diff:** `git diff --numstat` over `ledger/claims.json`, the mirrored file, the manifest and this README returns three rows and `claims.json` is not among them, so not one generated pin's `expect` changed. |
| [`d834f10`](../../commit/d834f10) | **Two records moved in one night, and five other pins moved with no bound behind them** — the sharpest case yet for reading a generated pin as a listing position rather than a record. The **bounded prime gap constant** fell `240 → 186`, cited to an OpenAI preprint of 30 August establishing `DHL[40,2]`; the **Steiner ratio** lower bound rose `0.8559 → 0.860*`, which upstream marks *unverified*. Both were checked against the artifacts rather than against the preprints' summaries of themselves: `openai/PrimeGaps186`'s `formalization.yaml` records `sorry_count: 0` for both named theorems, three non-standard axioms beside Lean's own three, and a review block reading `self-assessed` with "No independent human semantic review"; the Steiner Zenodo record carries, in its own description, the certificate counts upstream quotes — 330,193,755 records re-verified, 1,104,177,103 regions at the exact rational `43/50`. Meanwhile **five pins moved with no number behind them**: `47a` and `53a` were pure row reorders (the set-diff said so and the hand diff confirmed it), `41a` gained an asterisk, `50a` lost a row to prose, `8a` gained an escaping fix and a justification. And a **hand claim broke on a rounding direction**: upstream changed Haugland's minimum-overlap bound from `0.380927` to `0.380926` — the truncation, which is strictly below the `0.3809268534330870` the cited paper proves, re-fetched that day with "minimum overlap" and "Haugland" in the same response as the control. |
| [`ce5a57c`](../../commit/ce5a57c) | **The maintainer cleared a queue**: seventeen pull requests merged in about sixteen minutes, one of them ours, and sixteen mirrored files moved at once. **Several records moved**, each read against its source before the mirror did: Spencer's discrepancy constant `7/√17 → 5/√8` from an 8×8 matrix whose discrepancy we recomputed over all 256 sign vectors, and the leading coefficient of the cited upper-bound theorem corrected *upward* `3.7 → 4.1` by the authors' own erratum (the theorem reads `4.1√n + O(1)`); the bidisc Bohr radius `0.3174541 → 0.302825279492`; the Shannon capacity of the 7-cycle through four papers in six weeks to `3.25883262…`; the Furstenberg–Sárközy exponent past `3/4` to `0.75806770413`; and the `≈ 0.733412` exponent (a modulus-205, twelve-residue construction) re-attributed from Lewko to Beigel–Gasarch, who published it first. The pins moved on the **bounded prime gap constant** too, to `240` — and there the record did *not* move: it is still `186`, and upstream now lists rows in date order, so the last row is simply the newest. Reading the sources also turned up four small defects in text merged that day — a dead verification link, a quotation that is not verbatim, an asterisk on one page and not the other, and two control characters where a `\approx` was meant — recorded and not sent. |
...
### Reading our own records against their sources

Everything above catches upstream **changing**. All of it is structurally blind to upstream being wrong and *staying* wrong: a row that never moves fires nothing, ever. So since 2026-09-10 cited bound rows are read against the source each one cites. The reads that count toward coverage are **drawn by position**, in slices of five since 2026-09-11, at positions `node scripts/depth-audit.mjs --draw <position…>` re-prints for anyone (`--draw 375 425 475 525 575` is the most recent slice); rows read because something already looked wrong are labelled as such and counted apart. What the stored fingerprints prove is which row a verdict belongs to. That the positions were fixed before the sources were opened is this repository's own account of the order, which nothing in the artifact independently establishes — the audit says so itself, in the sampling note it prints above its own result.

`node scripts/depth-audit.mjs`, as of 2026-09-26: **34 rows audited — 19 sound, 1 defective, 6 unresolved, 8 unreachable.** Twenty-five were drawn by position from the 673 cited bound rows (of 770 in all, across the 115 constant files) that `node scripts/depth-audit.mjs --corpus` enumerates and defines; the other nine were chosen because something already looked wrong, and are counted apart from the coverage figure for exactly that reason. **Neither figure is a rate, and neither is projected onto the corpus** — 34 rows settle nothing about the rest, and the audit prints what it is blind to beside its own result.

The verdicts mean what they say, and the cited source is not always a paper — two of the sound rows were settled against a code repository, and one by recomputing a certificate printed in the constant's own file. *Sound*: the source was read and it supports the row. *Unresolved*: the source was read and does not settle the row — the whole content of that verdict is that it is still open. *Defective*: the source was read and it does not support the row. *Unreachable*: the source could not be read at all, and the eight of those are their own small finding about how checkable this literature is — four publisher bot-walls, a publisher serving a redirect page instead of the paper, a DOI resolver returning 502, a host that refused the connection, and one citation URL that served a "Site not found" page when we read it on 2026-09-05. Upstream replaced that link with the arXiv copy on 2026-09-26, through [`teorth/optimizationproblems#194`](https://github.com/teorth/optimizationproblems/pull/194), and that day the old URL was serving the paper again, so it was down and then up rather than permanently dead.

**The one defective row is about attribution, not the number.** Constant [46a](c/46a.html) credits the bound 58/15 to Bourgain's 1991 paper in *Geometric and Functional Analysis*. The published text of that paper proves 31/8 and states 58/15 on no page. The 58/15 is proved in Bourgain's companion paper, IHES preprint M/90/74 (Proposition 2.15).

**And the verdict counts are narrower than "nothing else is wrong".** A defective verdict means an audited row was not supported by the source it cites. A citation that is wrong *about itself* — a year, a title, a co-author's name, a quotation following a preprint rather than the journal the entry credits — is a real finding and is not what that verdict counts.
...
Step 1 is the two checks that need nothing but a network connection, and it is the fastest read. `npm run check` runs those two plus nine more (an entry-point guard, the re-derivation of the small Ramsey numbers table from its pinned source, the README state block, the rendered index page, the per-constant pages, the Ramsey page, a notation-coverage check, the deferral gate, and a check on our own David-facing brief) — it needs no account either, and **its exit code means what it says**: 0 is green, non-zero is a real finding.
...
- **116** mirrored files — 115 constant files plus upstream's repo-root `README.md` — byte-identical to upstream [`teorth/optimizationproblems`](https://github.com/teorth/optimizationproblems) at `2c1968c` (an upstream sha; it does not exist in this repo). The README is in scope because it is where upstream declares which bounds it stands behind: a bound demoted to "verification at minimal levels" leaves every constants file unchanged.
- **244 pinned claims** — 230 generated (one per bounds table, asserting the last-listed row's **position**, never "the record") and 14 hand-written. 2 are `manual: true` and therefore report UNVERIFIED by design: their source blocks automated fetch from CI runners.
...
Indicators, which never gate a build and never run in CI — and which refuse rather than print a figure they cannot stand behind, because a zero from a dead probe is indistinguishable from a measured one: `npm run depth-audit` (what has been read against its own sources, above), `npm run catches` (record movements per week — a ceiling, and it cannot see the Ramsey area at all), `npm run reports` (outside reports arriving through the per-row links on the page).
...
- **The metric this project judges itself by — externally-acknowledged corrections — is 2.** The first: five citation keys, submitted as a pull request and merged by the maintainer of the adopted repository on 2026-08-11. The second: a reference cited in two constants' tables, whose URL served a "Site not found" page on 2026-09-05 and 2026-09-21, opened upstream as [`teorth/optimizationproblems#194`](https://github.com/teorth/optimizationproblems/pull/194) on 2026-09-21 and merged by the maintainer on 2026-09-26. **The maintainer corrected our premise when merging:** the old URL was serving the paper again that day, so it had been down and then up, and the change was accepted because a project page that goes up and down is not a stable thing to cite, not because the link was dead. A third correction, sent by email to a different site on 2026-07-24 after an adversarial review that cut one claim, is **still unanswered**; the channels are separate and neither merge says anything about the email.
- **Two adopted surfaces.** This is not a survey of mathematical records; it is a deep watch on a small, named set.
- **Known limit:** this ledger checks that a cited record still says what it said, and that the surfaces citing it agree. It does not verify the mathematics behind any record — the depth audit above reads a sampled row against the source it cites, which is a different and much weaker thing than checking a proof. Where a bound is supported by a claimed proof that has not been refereed, the ledger tracks the row and upstream's own hedging — it does not adjudicate the proof.
